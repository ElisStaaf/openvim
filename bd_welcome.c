...
void bd_welcome_draw(bd_view_t *view) {
  ...
  const char *title = "Welcome to the OpenVim editor!";
  
  io_cursor((bd_width - strlen(title)) / 2, 3);
  io_printf(IO_UNDERLINE "%s" IO_NORMAL, title);
  
  int y = io_printf_wrap(2, bd_width - 4, 5,
                         "OpenVim is an open source, multifunctional editor and environment, with purposes ranging from file exploring to coding and note taking. "
                         "Its name comes from the \"Vim\" editor, combined with \"Open\".\n\n"
                        );
